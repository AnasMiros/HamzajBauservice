...
export default class Autoplay {
	...
	switch() {
		if ( this.isEnabled() ) {
			this.getPlayer().navigation.autoButton.children[ 0 ].id =
				'journey-auto';
	...
	start() {
		if ( this.isEnabled() ) {
			...
			const timeout = this.getPlayer().current.timeout | 5000;
			...
			const currentOnStart = this.getPlayer().current;
			setTimeout( () => {
	 ...
	 */
	guide( timeout ) {
		if ( this.getPlayer().current.type !== 'guide' || ! this.isEnabled() )
			return;

		if ( this.getPlayer().guide.next() ) {
			const indexOnStart = this.getPlayer().guide.currentIndex;
			this.getPlayer().guide.showAutoplayBar( timeout );
			setTimeout( () => {
				if ( indexOnStart !== this.getPlayer().guide.currentIndex )
					return;
				this.guide( timeout );
			}, timeout );
		}
