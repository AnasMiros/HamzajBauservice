...
export default class Bubble {
	...
	update() {
		...
		const wpAdminBarRect = document
			.querySelector( '#wpadminbar' )
			.getBoundingClientRect();
		const sourceRect = this.focus.getHtmlElement().getBoundingClientRect();
		const parentRect = this.parent.getBoundingClientRect();

		const viewHeight =
			window.innerHeight || document.documentElement.clientHeight;

		const bubbleRect = this.htmlElement.getBoundingClientRect();

		if (
			parentRect.width - sourceRect.right > 350 &&
			sourceRect.top - wpAdminBarRect.height + bubbleRect.height <
				viewHeight
		) {
			this.htmlElement.className = 'bubble-left';

			this.htmlElement.style.top =
				sourceRect.top - wpAdminBarRect.height + 'px';
			this.htmlElement.style.left =
				sourceRect.right + this.distance + 'px';
		} else if (
			sourceRect.left > 350 &&
			sourceRect.top - wpAdminBarRect.height + bubbleRect.height <
				viewHeight
			...
			this.htmlElement.style.top =
				sourceRect.top - wpAdminBarRect.height + 'px';
			this.htmlElement.style.left =
				sourceRect.left - this.htmlElement.scrollWidth + 'px';
		} else if (
			sourceRect.top > 350 &&
			...
			this.htmlElement.style.left =
				sourceRect.left +
				sourceRect.width / 2 -
				this.htmlElement.scrollWidth / 2 +
				'px';
			this.htmlElement.style.bottom =
				parentRect.height - sourceRect.top + this.distance + 'px';

			this.htmlElement.style.position = 'fixed';
		} else if (
			parentRect.height - sourceRect.bottom > 350 &&
			sourceRect.x - bubbleRect.width / 2 + sourceRect.width / 2 > 0
		) {
			...
			this.htmlElement.style.top =
				sourceRect.top + sourceRect.height - this.distance + 'px';
		} else {
			this.htmlElement.className = '';

			this.htmlElement.style.margin = 'auto';
			this.htmlElement.style.left =
				parentRect.width / 2 - bubbleRect.width / 2 + 'px';
	...
	showAutoProgress( timeout = 5000 ) {
		...
		this.autoProgress.style.animation =
			'circle-animation ' + timeout / 1000 + 's linear';
		this.autoProgress.style.clipPath =
