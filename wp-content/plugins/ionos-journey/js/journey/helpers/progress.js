...
export default class Progress {
	...
	static init( progressString, userId ) {
		this.userId = userId;
		if (
			localStorage.getItem( Progress.progressKey + this.userId ) === null
		) {
			this.version = this.versionConstraint;
			return;
		}
		if (
			localStorage.getItem( Progress.progressKey + userId ).length > 0
		) {
			const progressData = JSON.parse(
				localStorage.getItem( Progress.progressKey + this.userId )
			);
	...
	static store( progress ) {
		if ( Date.now() > this.timestamp ) {
			...
			localStorage.setItem(
				Progress.progressKey + this.userId,
	...
	static clear() {
		localStorage.removeItem( Progress.progressKey + this.userId );
	}
