export default class UrlHelper {
	...
	static redirectToPage( page, urlParams ) {
		const params =
			urlParams instanceof Array
				? this.convertToJson( urlParams )
				: urlParams;
		if ( ! Object.prototype.hasOwnProperty.call( params, 'wp-tour' ) ) {
			params.wp_tour = 'started';
		}

		const pageName = page.includes( '.php' ) ? page : page + '.php';
		let url = this.getUrl().toString().includes( '?' )
			? new URL( this.getUrl().toString().split( '?' )[ 0 ] )
			: this.getUrl();

		if ( window.location.pathname.split( '/' ).pop().length > 1 ) {
			url = new URL(
				url.toString().replace( this.getPageName(), pageName )
			);
		} else {
